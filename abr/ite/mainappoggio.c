...
void verifica_abr_uguali(ABR *albero, LISTA *lista, int current_ABR, int num_ABR, FUNC funz) {
	...
	if(estrai_lista(lista, op)->tipo != funz.tipo) {
		...
		if(ABR_uguali_iterativo(albero,estrai_lista(lista, op)->content, funz) == false) {
			printf("non ");
		}
		printf("sono uguali!\n");
	}	
...
void array_ordinato(ABR *albero, FUNC funz) {
	void **array;
	int i, num_nodi;
	num_nodi = nnodi_iterativo(albero);
	array = (void **) malloc(num_nodi * sizeof(void *));
	riempi_array_da_ABR_iterativo(albero, array, funz);
...
ABR *esegui_es5(ABR *albero, FUNC funz) {
	...
	printf("\nElemento B\n");
	b = funz.getInput();
	albero = es5_iterativo(albero, op, a, b, funz);
	if(funz.libera != NULL) {
		funz.libera(a);
	}
	if(funz.libera != NULL) {
...
ABR *cancella_elem(ABR *albero, FUNC funz) {
	...
	albero = cancella_valore_iterativo(albero, elem, funz);
	if(funz.libera != NULL) {
		funz.libera(elem);
	}
	printf("\nOperazione terminata con successo!\n");
...
ABR *inserisci_casuali(ABR *albero, FUNC funz) {
	...
	albero = insrand_iterativo(albero, op, funz);
	printf("\nOperazione terminata con successo!\n");
	return albero;
}
ABR *inserisci_elem(ABR *albero, FUNC funz) {
	void *elem = funz.getInput();
	albero = inserisci_iterativo(albero, elem, funz);
	if(funz.libera != NULL) {
		funz.libera(elem);
	}
	printf("\nOperazione terminata con successo!\n");
...
void termina(LISTA *lista, FUNC funz) {
	LISTA *tmp;
	while(lista != NULL) {
			lista->content = distruggi_iterativo(lista->content, funz);
			tmp = lista;
			lista = lista->next;
			free(tmp);
	}
