...
//DOMANDA BFS PER labyrinth PESATO?
void BFS(struct labyrinth *graph, int *s, int **dist, int **col, int ***pred) { //non riesco ad allocare dist pred col qua dentro
	struct coda *Q;
	void *u;
	int i, j, k, x, y;
	int dir;
	...
	for(i=0;i<graph->righe; i++) {
		for(j=0;j<graph->colonne;j++) {
			...
			if(graph->maze[i][j] == '|' || graph->maze[i][j] == '-' || graph->maze[i][j] == '#' || graph->maze[i][j] == 'D') { // Non puoi passare per il muro
				col[i][j] = 2;
			}
		}
	}
